            ...
            if len(words_list) == 0:
                ...
                with col2:
                    ...
                    if mode == "Inflections AND Derivations (Combined)":
                        estimated_batches *= 3  # Triple for combined mode (inflections + derivations + derivation inflections)
                ...
                if process_button and api_key:
                    ...
                    if mode == "Inflections only (e.g., analyze → analyzing, analyzed)":
                        ...
                        # 1. Get Inflections of original words
                        st.subheader("🔄 Processing Inflections (Step 1 of 3)...")
                        with st.spinner("Calling GPT-4o for inflections..."):
                            inflection_results = process_vocabulary_batch(
                                words_list, api_key, generate_lemmatization_prompt
                            )
                        
                        # 2. Get Derivations of original words
                        st.subheader("🔄 Processing Derivations (Step 2 of 3)...")
                        with st.spinner("Calling GPT-4o for derivations..."):
                            derivation_results = process_vocabulary_batch(
                                words_list, api_key, generate_derivation_prompt
                            )
                        
                        # 3. Get Inflections of all derivations
                        st.subheader("🔄 Processing Inflections of Derivations (Step 3 of 3)...")
                        
                        # Collect all unique derivations (excluding the original words)
                        all_derivations = set()
                        original_words_set = set(words_list)
                        
                        for item in derivation_results:
                            for variation in item.get('variations', []):
                                if variation.lower() not in [w.lower() for w in original_words_set]:
                                    all_derivations.add(variation)
                        
                        derivations_list = sorted(list(all_derivations))
                        st.info(f"Found {len(derivations_list)} unique derivations to inflect")
                        
                        # Get inflections of all derivations
                        derivation_inflections = []
                        if derivations_list:
                            with st.spinner(f"Calling GPT-4o for inflections of {len(derivations_list)} derivations..."):
                                derivation_inflections = process_vocabulary_batch(
                                    derivations_list, api_key, generate_lemmatization_prompt
                                )
                        
                        # 4. Merge ALL the results
                        st.subheader("🔀 Merging all results...")
                        ...
                        # Collect all inflections of derivations
                        derivation_inflection_vars = set()
                        for item in derivation_inflections:
                            derivation_inflection_vars.update(item.get('variations', []))
                        ...
                        for word in all_originals:
                            ...
                            # Add all inflections of derivations
                            combined_vars = sorted(list(inf_vars | der_vars | derivation_inflection_vars))
